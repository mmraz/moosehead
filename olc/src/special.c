...
void bounty_admin_claim(CHAR_DATA *ch, CHAR_DATA *shade, OBJ_DATA *obj)
{
  ...
  if(ch->pcdata->quest_wins[QUEST_BOUNTY_CLAIM] == 4)
  {
...
void describe_mob_bounty(CHAR_DATA *ch, CHAR_DATA *teller)
{
  char buf[256];
  ...
  {// Reward lowered, extra tip added
    ...
    if(bounty_type == BOUNTY_MOB_DESC)
      ...
      sprintf(buf, "The item that was seen: {W%s{x\n\r", item->short_descr);
    send_to_char(buf, ch);
  }
  else
  {
    switch(bounty_type)
    {
      ...
      case BOUNTY_ITEM_NAME:
        sprintf(buf, "Only an item was spotted: {W%s{x\n\r", item->short_descr);
        break;
      ...
      case BOUNTY_ROOM_DESC:
        send_to_char("Our scout was too nervous to get close, but described a {Wroom{x:\n\r", ch);
  ...
  {
    switch(bounty_desc % 7)
    {
      case 0: send_to_char("Eliminate it swiftly and you will be rewarded.'\n\r", ch);
        break;
      case 1: send_to_char("Bring its ear back to me and you will be rewarded.'\n\r", ch);
        break;
      case 2: send_to_char("You will be suitably rewarded if you kill it soon.'\n\r", ch);
        break;
      case 3: send_to_char("Kill it for us then you may claim your reward.'\n\r", ch);
        break;
      case 4: send_to_char("Find it and end its life, then bring the proof to me.'\n\r", ch);
        break;
      case 5: send_to_char("Bring proof that you have eliminated it and I will reward you.'\n\r", ch);
        break;
      case 6: send_to_char("Hurry, the chance to kill it for a reward will not last forever.'\n\r", ch);
        break;
    }
  }
  if(ch->level >= 59)
    ...
    if(sit && sit->people)
    {
      for(target = sit->people; target; target = target->next_in_room)
      {
    		...
    		if(target->position != POS_SITTING)
    		  continue;
    		if(target->pcdata->quest_wins[QUEST_BOUNTY_CLAIM] >= 4 || IS_IMMORTAL(target))
    		{
          describe_mob_bounty(target, NULL);
          shade->qnum2 = 5;
        }
      }
    }
		...
		{// Tell anyone sitting -- wait 5 rounds before it can happen
		  for(; target; target = target->next_in_room)
		  {
		    if(!IS_NPC(target) && target->position == POS_SITTING)
    		  describe_mob_bounty(target, shade);
    	}
		}
		break;
	}

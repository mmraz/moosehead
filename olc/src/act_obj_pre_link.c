    ...
    for ( paf = obj->affected; paf != NULL; paf = paf->next )
    {
      ...
      {/* No match, let's see what it is */
        if(obj->item_type == ITEM_WEAPON)
        {
          ...
          if(paf->location == 0 && paf->bitvector == WEAPON_HOLY)
            continue;
          if(paf->location == TO_WEAPON && paf->bitvector == WEAPON_SHARP)
            continue;
...
int check_repair_obj(OBJ_DATA *obj, CHAR_DATA *ch, CHAR_DATA *weapon, CHAR_DATA *armor, bool verbose)
{
	...
	if(obj->item_type == ITEM_WEAPON)
		victim = weapon;
	else if(obj->item_type == ITEM_ARMOR || obj->item_type == ITEM_CLOTHING || obj->item_type == ITEM_CONTAINER || obj->item_type == ITEM_DRINK_CON || obj->item_type == ITEM_JEWELRY || obj->item_type == ITEM_LIGHT)
		victim = armor;
	else
	{
		if(verbose)
	...
	cost = UMAX(cost, 10);// Cover those dirt cheap items
	return cost;
}
...
void get_obj( CHAR_DATA *ch, OBJ_DATA *obj, OBJ_DATA *container )
{
  ...
  &&  get_trust(ch) < obj->level)
  {
      ...
      if ((container->pIndexData->vnum == OBJ_VNUM_PIT || container->pIndexData->vnum == OBJ_VNUM_MATOOK_PIT || container->pIndexData->vnum == OBJ_VNUM_DEMISE_PIT || container->pIndexData->vnum == OBJ_VNUM_HONOR_PIT || container->pIndexData->vnum == OBJ_VNUM_POSSE_PIT || container->pIndexData->vnum == OBJ_VNUM_ZEALOT_PIT || container->pIndexData->vnum == OBJ_VNUM_WARLOCK_PIT || container->pIndexData->vnum == OBJ_VNUM_FLUZEL_PIT)
...
{
  int i;
  send_to_char("Your gear returns to you.\n\r", ch);
  ...
  for(item = ch->carrying; item; item = item->next_content)
  {
    if(item->wear_loc == WEAR_HIDDEN)
    {
      ...
      if(item->damaged == 1000)
        act("$n is shattered and will cost extra to repair.", ch, item, NULL, TO_CHAR, FALSE);
    }
  }
...
int get_link_limit(CHAR_DATA *ch)
{
  ...
  if(IS_IMMORTAL(ch))
    return LINK_MAX;
  if(!ch->pcdata->clan_info || IS_SET(ch->act, PLR_DWEEB))
    return 0;
...
void do_link(CHAR_DATA *ch, char *argument)
{
  ...
  if(obj->item_type != ITEM_WEAPON &&
    ...
    obj->item_type != ITEM_CONTAINER && obj->item_type != ITEM_DRINK_CON &&
...
void do_unlink(CHAR_DATA *ch, char *argument)
{
  ...
  /* Require 'unlink <obj> override' if it would shatter it to unlink it */
  mod = 0;
  ...
  switch ( container->item_type )
  {
    ...
    if(IS_SET(container->extra_flags,ITEM_CLAN_CORPSE) && is_clan(ch) && !IS_IMMORTAL(ch))
    {
      ...
      {/* Check for a friendly return attempt */
        ...
        {
          owner->pcdata->corpse_timer = UMAX(owner->pcdata->corpse_timer - 1, 0);
          WAIT_STATE(ch, PULSE_VIOLENCE * 2);
          if(!owner->pcdata->corpse_timer)
          {
      ...
      if(!loot)
      {
        send_to_char("You did not participate in the kill, you can't take the loot.\n\r", ch);
        return;
      }
      if(loot->damage <= 0 && loot->duration <= 0)
      {
        send_to_char("All the items you could take have already been looted.\n\r", ch);
        return;
      }
      ...
      {/* Not just taking coins, there must be a search type */
        ...
        if(!loot->type)
        {
          send_to_char("You must search the victim's corpse before you may loot it.\n\r", ch);
          return;
        }
        if(loot->type == RARITY_IMPOSSIBLE)
          type = RARITY_UNCOMMON | RARITY_RARE | RARITY_GEM | RARITY_IMPOSSIBLE;
        else
          type = loot->type;
        prev = NULL;
        ...
        /* Check if rarity matches, then check if name matches, then check if count is equal */ 
          if(IS_SET(looting->rarity, type) && can_see_obj(ch, looting) && is_name(arg1, looting->name))
          {
            found++;
            if(found == index)
            {
      ...
      if(container->owner && str_cmp(container->owner, ch->name))
        obj->stolen_timer = UMAX(obj->stolen_timer, 45);
      return;
    }
  ...
  if ( str_cmp( arg1, "all" ) && str_prefix( "all.", arg1 ) )
  {
	...
	return;
         ...
         if(found && victim != ch && obj->contains != NULL) 
	 {
	    ...
	    {
	       if(ch->clan != victim->clan || ch->clan == nonclan_lookup("loner")
...
void do_put( CHAR_DATA *ch, char *argument )
{
  ...
  &&  !CAN_WEAR(container,ITEM_TAKE))
  {
    ...
    SET_BIT(obj->extra_flags,ITEM_HAD_TIMER);
	  ...
	  obj->timer = 3200;//Around 36 hours
   }

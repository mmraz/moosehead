...
HELP_TRACKER *  help_tracks[27]; // A-Z and #
...
void select_bounty(int qualifier)
{
  ...
  {// If qualifier is 0 just go with the minimum level of 0
    ...
    if(qualifier > 0 && selection_mod < 80)
    {
      if(selection_mod < 60)
        qualifier = 0;
      else
    ...
    for(; pMobIndex; pMobIndex = pMobIndex->next)
    {
      ...
      for ( mob = char_list; mob != NULL; mob = mob->next )
      {
        if(IS_NPC(mob) && mob->pIndexData == pMobIndex)
        {
          ...
          if(iClass != MAX_CLASS)
            continue;// No good, in guild room
          break;// Mob is good
        }
      }
...
char buf[256];
sprintf(buf, "%d level 0 bounties marked.", bounty_available[0]);
log_string(buf);
  select_bounty(0);
}
...
#ifdef OLC_VERSION
/* Just in case it doesn't get loaded */
  strcpy(recipe_file, "/mud/moosehead/olc/olcarea/recipes.are");
#endif
...
if(help_first || help_last)
bug("FIRST OR LAST HELP IS NOT NULL", 0);
...
log_string(strArea);
  ...
  create_help_tracks();// Link the help tracks together
  mark_mob_bounties();
		...
		for ( i = 0 ; i < MAX_IN_RECIPE ; i++ )
		{
			...
			if (num != 0 )
                ...
                if(fread_number(fp) == 1)
                  ...
                  recipe->active = FALSE;
                fread_to_eol(fp);
                read_to_tilde(fp, &recipe->cmessage);
                fread_to_eol(fp);
                read_to_tilde(fp, &recipe->rmessage);
  ...
  for(pHelp = help_first; pHelp; pHelp = pHelp->next)
  {
    ...
    //#endif
      ...
      else
      {
        ...
        while(phWalker->next && 
          alphabet_before(phWalker->keyword, phTracker->keyword))
        ...
        {// Middle of the list -- needs to go in before this one
          phTracker->prev = phWalker->prev;
          phTracker->next = phWalker;
          phWalker->prev->next = phTracker;
          phWalker->prev = phTracker;
        }
      }
...
void write_new_helps(void)
{
  rename(HELP_FILE, HELP_BAK);// Back it up before saving over it
  FILE *fp = fopen(HELP_FILE, "w");
  HELP_DATA *help = help_first;
  if(!fp)
  {
    bug("Unable to open help file for writing to.\n\r", 0);
    return;
  }
  while(help)
  {
    fprintf(fp, "%d %s~\n", help->level, help->keyword);
    fprintf(fp, "%s~\n", help->related ? help->related : "none");
    fprintf(fp, "%ld %d %s\n", help->modified,
      help->status & HELP_STAT_BASICS ? help->status & HELP_STAT_BASICS : HELP_STAT_LEGACY,
      help->editor ? help->editor : "none");
    if(help->text)
      fprintf(fp, "%s~\n", help->text);
    else
      fprintf(fp, "~\n");

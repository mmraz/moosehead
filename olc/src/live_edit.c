...
{/* Stops editing, in its own function so it can be easily used for if a player quits */
  if(ch->pcdata == NULL || !ch->pcdata->edit_flags)
    return;
  if(IS_SET(ch->pcdata->edit_flags, EDITMODE_RULES))
  {
...
void blast_punctuation(char *arg, bool leave_quote, bool capitalize)
{
  int i, j;
...
int calc_cost_range(int start, int count, bool hedit)
{
  ...
  {
    cost += GET_PRICE(i, hedit);
  }
  return cost;
}

/* DO NOT OVERRIDE UNLESS DOING A PRICE CORRECTION */
...
{
  if(IS_SET(obj->type, (PLAN_PLACED | PLAN_PREVIEWED)) && !override)
  {
...
void load_item_obj(PLAN_DATA *obj, bool strings)
{
  ...
  pObjIndex->extra_flags = ITEM_NOLOCATE;
    ...
    for ( object = object_list; object != NULL; object = obj_next )
    {
      ...
      if(object->pIndexData->vnum == obj->plan_index * -1)
      {
        ...
        while(object->contains)
        {
          ...
          obj_from_obj(remove);
          obj_to_room(remove, object->in_room);
        }
        extract_obj(object);
      }
    }
...
void player_edit(CHAR_DATA *ch, char *argument, bool hedit)
{
  ...
  if(arg[0] == '\0')
  {
    ...
    {/* Prevent accidental quits */
      if(hedit)
      {
        send_to_char("{WAvailable commands{x: ({Y*{x means they have a cost or refund)\n\r", ch);
        send_to_char("list display create clone edit delete {Y*place *upgrade *replace *(un)link{x stop\n\r", ch);
        send_to_char("swap check charter rules\n\r", ch);
        if(ch->pcdata->edit_obj)
        {
          switch(ch->pcdata->edit_obj->type & PLAN_MASK_TYPE)
          {
            case PLAN_ROOM: send_to_char("{WRoom Edit{x: label name desc hregen mregen magelab altar outdoor dark\n\r", ch);
              break;
            case PLAN_ITEM: send_to_char("{YItem Edit{x: label name short desc type target regen liquid hidden\n\r", ch);
              break;
            case PLAN_MOB: send_to_char("{BMob Edit{x: label name short long desc race align gender discount level\n\r", ch);
              break;
...
/*    if(!str_prefix(arg, "hidden"))
  ...
  if(!str_prefix(arg, "edit"))
  {
    ...
    if(IS_SET(ch->pcdata->edit_flags, (EDITMODE_CHARTER | EDITMODE_RULES)))
    {
      if(IS_SET(ch->pcdata->edit_flags, EDITMODE_CHARTER))
        send_to_char("Save or cancel your hedit charter first.\n\r", ch);
      else
        send_to_char("Save or cancel your hedit rules first.\n\r", ch);
      return;
    }
    argument = one_argument(argument, arg);
    if(arg[0] == 0)
...
/*      if(obj->to_place && ((ROOM_INDEX_DATA*)obj->to_place)->people)
      ...
      if(room == ch->pcdata->clan_info->clan->hall)
      {
        send_to_char("You can't remove the entry room for your hall.\n\r", ch);
        return;
      }
      SET_BIT(obj->flags, PLAN_ROOM_MARKED);/* Prevents it being entered by the search */
      search_linked_rooms(ch->pcdata->clan_info->clan->hall);
      /* After complete, scan through and verify all placed rooms are marked */
      start = START_OBJ(ch, hedit);
      for(; start != NULL; start = start->next)
      {
        if(IS_SET(start->type, PLAN_ROOM) &&
          IS_SET(start->type, (PLAN_PLACED | PLAN_PREVIEWED)))
        {
  ...
  if(!str_prefix(arg, "swap"))
  {
    ...
    {
      send_to_char("You do not have permissions to swap.\n\r", ch);
      return;
    }
    argument = one_argument(argument, arg);
    if(arg[0] == '\0')
    {
      if(hedit)
        send_to_char("Usage: hedit swap <current label> <new label>\n\r", ch);
      else
        send_to_char("Usage: pedit swap <current label> <new label>\n\r", ch);
      send_to_char("Mob and item types must match in order to swap.\n\r", ch);
      return;
    }
    if((obj = find_edit_obj(ch, arg, hedit)) == NULL)
    {
      sprintf(buf, "%s not found to swap out.\n\r", arg);
      send_to_char(buf, ch);
      return;
    }
    if(!IS_SET(obj->type, (PLAN_PREVIEWED | PLAN_PLACED)) || !obj->to_place)
    {
      send_to_char("You may only swap previewed or placed objects.\n\r", ch);
      return;
    }
    if(IS_SET(obj->type, PLAN_EXIT))
    {
      send_to_char("You may not swap exits.\n\r", ch);
      return;
    }
    ch_room = find_char_room_obj(ch, hedit);
    if((IS_SET(obj->type, PLAN_ROOM) && obj != ch_room) ||
    (!IS_SET(obj->type, PLAN_ROOM) && obj->loc != ch_room->plan_index))
    {
      send_to_char("You must be in the same room as the object you wish to swap spawns in.\n\r", ch);
      return;
    }
    argument = one_argument(argument, arg);
    if((new_obj = find_edit_obj(ch, arg, hedit)) == NULL)
    {
      sprintf(buf, "%s not found to swap in.\n\r", arg);
      send_to_char(buf, ch);
      return;
    }
    if(IS_SET(new_obj->type, (PLAN_PREVIEWED | PLAN_PLACED)))
    {
      send_to_char("The object to swap in may not be previewed or placed.\n\r", ch);
      return;
    }
    if((obj->type & PLAN_MASK_TYPE) != (new_obj->type & PLAN_MASK_TYPE))
    {
      send_to_char("The objects must be the same to swap.\n\r", ch);
      return;
    }
    if(IS_SET(obj->type, PLAN_ITEM) &&
      (obj->flags & PLAN_MASK_OBJ) != (new_obj->flags & PLAN_MASK_OBJ))
    {
      send_to_char("The item type must be the same to swap.\n\r", ch);
      return;
    }
    if(new_obj->editing && ch->pcdata->edit_obj != new_obj)
    {
      send_to_char("Someone else is editing that object, you may not swap it in.\n\r", ch);
      return;
    }
    if(IS_SET(new_obj->type, PLAN_ROOM))
    {
      if(new_obj->name == NULL || (new_obj->desc == NULL && ch->pcdata->edit_len == 0))
      {
        send_to_char("Name and desc must be set to swap in a room.\n\r", ch);
        return;
      }
    }
    else
    {
      if(new_obj->name == NULL || (new_obj->long_d == NULL) || new_obj->short_d == NULL ||
        (IS_SET(new_obj->type, PLAN_MOB) && new_obj->desc == NULL && ch->pcdata->edit_len == 0))
      {
        if(IS_SET(new_obj->type, PLAN_MOB))
          send_to_char("Name, desc, short and long must be set to preview a mob.\n\r", ch);
        else
          send_to_char("Name, short and long must be set to preview an item.\n\r", ch);
        return;
    ...
    {/* I'm sure there's a more efficient way to code this */
      /* Only swap strings for now */
      /*int old_cost = obj->cost;
      int old_opt0 = obj->opt[0];
      int old_opt1 = obj->opt[1];
      long old_type = obj->type;
      long old_flags = obj->flags;*/
      bool old_flagged = obj->flagged;
      char *old_name = obj->name;
      char *old_label = obj->label;
      char *old_short_d = obj->short_d;
      char *old_long_d = obj->long_d;
      char *old_desc = obj->desc;
      ...
      obj->opt[0] = new_obj->opt[0]; new_obj->opt[0] = old_opt0;
      obj->opt[1] = new_obj->opt[1]; new_obj->opt[1] = old_opt1;
      obj->type = new_obj->type; new_obj->type = old_type;
      obj->flags = new_obj->flags; new_obj->flags = old_flags;*/
      obj->flagged = new_obj->flagged; new_obj->flagged = old_flagged;
      obj->name = new_obj->name; new_obj->name = old_name;
      obj->label = new_obj->label; new_obj->label = old_label;
  ...
  if(hedit)
  {
    if(!str_prefix(arg, "charter"))
    {
      if(!IS_IMMORTAL(ch) && ch->pcdata->clan_info->rank != 5)
      {
        send_to_char("You can't edit the clan's charter.\n\r", ch);
        return;
      }
      if(IS_SET(ch->pcdata->edit_flags, EDITMODE_RULES))
      {
      ...
      if(!str_cmp(arg, "start"))
      {
        if(IS_SET(ch->pcdata->edit_flags, EDITMODE_CHARTER))
        {
          send_to_char("You are already editing your clan's charter.\n\r", ch);
          return;
        }
        if(ch->pcdata->edit_obj)
        {
        ...
        start_long_edit(ch, MAX_NOTE_DESC, LONG_EDIT_DESC, ch->pcdata->clan_info->clan->charter);
        send_to_char("Charter editing begun, please see 'help text' for editing commands.\n\r", ch);
        SET_BIT(ch->pcdata->edit_flags, EDITMODE_CHARTER);
      }
      else if(!str_cmp(arg, "cancel"))
      {
        if(!IS_SET(ch->pcdata->edit_flags, EDITMODE_CHARTER))
        {
          send_to_char("You are not editing your charter, you can't cancel edits on it.\n\r", ch);
          return;
        }
        REMOVE_BIT(ch->pcdata->edit_flags, EDITMODE_CHARTER);
        end_long_edit(ch, NULL);
        send_to_char("Charter editing cancelled, no modifications made.\n\r", ch);
      }
    ...
    if(!str_prefix(arg, "rules"))
    {
      if(!IS_IMMORTAL(ch) && ch->pcdata->clan_info->rank != 5)
      {
        send_to_char("You can't edit the clan's rules.\n\r", ch);
        return;
      }
      if(IS_SET(ch->pcdata->edit_flags, EDITMODE_CHARTER))
      {
      ...
      if(!str_cmp(arg, "start"))
      {
        if(IS_SET(ch->pcdata->edit_flags, EDITMODE_RULES))
        {
          send_to_char("You are already editing your clan's rules.\n\r", ch);
          return;
        }
        if(ch->pcdata->edit_obj)
        {
        ...
        start_long_edit(ch, MAX_NOTE_DESC, LONG_EDIT_DESC, ch->pcdata->clan_info->clan->rules);
        send_to_char("Rules editing begun, please see 'help text' for editing commands.\n\r", ch);
        SET_BIT(ch->pcdata->edit_flags, EDITMODE_RULES);
      }
      else if(!str_cmp(arg, "cancel"))
      {
        if(!IS_SET(ch->pcdata->edit_flags, EDITMODE_RULES))
        {
          send_to_char("You are not editing your rules, you can't cancel edits on them.\n\r", ch);
          return;
        }
        end_long_edit(ch, NULL);
        send_to_char("Rules editing cancelled, no modifications made.\n\r", ch);
        REMOVE_BIT(ch->pcdata->edit_flags, EDITMODE_RULES);
      }
      else if(!str_cmp(arg, "save"))
      {
  ...
  }/* End if hedit for rules and charter */

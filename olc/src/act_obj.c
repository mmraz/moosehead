          ...
          if(IS_SET(looting->rarity, loot->type) && can_see_obj(ch, looting) && is_name(arg1, looting->name))
          {
            ...
            if(found == index)
            {
              ...
              while(looting->contains)
              {
                ...
                obj_to_obj(moving, looting->in_obj);
              }
...
void do_eat( CHAR_DATA *ch, char *argument )
{
  ...
  if ( obj->item_type == ITEM_PILL && ch->position <= POS_RESTING )
  {
    ...
    if(obj->value[4] <= 0 || obj->item_type != ITEM_FOOD)
      act( "You eat $p.", ch, obj, NULL, TO_CHAR ,FALSE);
    else
      act("You eat $p. It's extra juicy.", ch, obj, NULL, TO_CHAR, FALSE);
  ...
  if  ( !IS_NPC(ch) )
  {
      ...
      else
      {
        if(obj->value[4] > 0)
        {
          ...
          if ( ( liquid = obj->value[2] )  < 0 )
          {
              bug( "Do_eat: bad liquid number %d.", liquid );
              liquid = obj->value[2] = 0;
          }
          amount = liq_table[liquid].liq_affect[4];
          amount = UMIN(amount, obj->value[4]);
          gain_condition( ch, COND_DRUNK,
            amount * liq_table[liquid].liq_affect[COND_DRUNK] / 36 );
          gain_condition( ch, COND_FULL,
            amount * liq_table[liquid].liq_affect[COND_FULL] / 4 );
...
void do_forge ( CHAR_DATA *ch, char *argument )
{
      ...
      while(recipe)
      {
        ...
        if(recipe->active && (recipe->vnum_container == forge->pIndexData->vnum || recipe->vnum_container == -1))
        {
          ...
          /* Drop out of the loop if there's still an obj -- extra ingredients */
          if(!obj)
          {
            for ( j = 0 ; j < count; j++ )
            {
        ...
        else
        {
           ...
           if ( recipe->skill_sn > 0 )
            ...
            else if(recipe->difficulty > 0)
            {
              if ( number_percent() < recipe->difficulty )
              {
                  bFail = TRUE;
              }
            }
        ...
        if ( complete == NULL )
        {
          ...
          if(recipe->rmessage)
            act(recipe->rmessage, ch, forge, NULL, TO_ROOM, FALSE);
          else
            act("$p glows with a magical light!", ch, forge, NULL, TO_ROOM, FALSE);
          if(recipe->cmessage)
            act(recipe->cmessage, ch, forge, NULL, TO_CHAR, FALSE);
          else

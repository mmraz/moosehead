          ...
          if(is_bounty_target(victim, TRUE))
          {
            ...
            if(killer && !IS_NPC(killer))
            {
              ...
              killer->pcdata->quest_wins[QUEST_BOUNTY_KILL]++;
            	...
            	sprintf(buf, "%s killed %s(%d) for a bounty (%d %d %d%s)", killer->name,
                victim->short_descr, victim->pIndexData->vnum, bounty_type,
                bounty_timer, bounty_complete,
              ...
              ear->value[3] = 0;
                ...
                {
                  ear->value[3] = 1;// Give a shade stone for this one
    ...
    case DAM_LIGHT:
      ...
      if(single_target)
      {
        ...
        if(IS_SET(victim->mhs,MHS_GLADIATOR))
          ...
          af.duration++;
        if(!IS_AFFECTED(victim,AFF_BLIND) && !saves_spell(grenade->level, victim, DAM_LIGHT))
        {
          affect_to_char(victim, &af);
      ...
      else
      {
        for (vch = ch->in_room->people; vch != NULL; vch = vch_next)
        {
          ...
          if(!IS_AFFECTED(vch,AFF_BLIND) && !saves_spell(grenade->level / 2, vch, DAM_LIGHT))
          {// Boom

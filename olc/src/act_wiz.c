...
{// HACK CHECK CODE
  ...
  {
    for(pObjIndex = obj_index_hash[i]; pObjIndex; pObjIndex = pObjIndex->next)
    {
      if(pObjIndex->item_type == ITEM_FOOD && (pObjIndex->value[2] || pObjIndex->value[4]))
      {
        sprintf(buf, "Object %s(%d) has value 3 or 5 set.\n\r", pObjIndex->short_descr, pObjIndex->vnum);
        send_to_char(buf, ch);
      } 
    }
  }
...
#ifdef OLC_VERSION
          if(d->character->pcdata && d->character->pcdata->ref_help &&
            IS_SET(d->character->pcdata->edit_help.status, HELP_STAT_MODIFIED))
            {
              fAny = TRUE;
              sprintf(buf, "%s currently has a modified help file open.\n\r",
        		   d->character->name);
              send_to_char(buf,ch);
            }
#endif

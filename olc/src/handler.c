...
void extract_char( CHAR_DATA *ch, bool fPull )
{
  ...
  return;
    ...
    if(ch->pcdata)
    {
      ...
      {// Release any help file in progress, assumed a discard
        ch->pcdata->ref_help->status &= HELP_STAT_BASICS;
        ch->pcdata->ref_help = NULL;
        clear_string(&ch->pcdata->edit_help.keyword, NULL);
        clear_string(&ch->pcdata->edit_help.text, NULL);
        clear_string(&ch->pcdata->edit_help.related, NULL);
        clear_string(&ch->pcdata->edit_help.editor, NULL);
      }
#endif
    }

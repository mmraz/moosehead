...
void parse_note( CHAR_DATA *ch, char *argument, int type )
{
  ...
  return;
    ...
    if(ch->pcdata->edit_type)
    {
      if(!ch->pnote)
      {
        send_to_char("You are not editing a note.\n\r", ch);
        return;
      }
      if(ch->pnote->type != type)
      {
        send_to_char("You are editing a different type of note.\n\r", ch);
        return;
      }
      if(argument[0] == '\0')
      {
        switch(type)
        {
          case NOTE_NOTE: send_to_char("Use 'note done' to stop editing.\n\r", ch); break;
          case NOTE_IMMORTAL: send_to_char("Use 'gnote done' to stop editing.\n\r", ch); break;
        	case NOTE_CLAN: send_to_char("Use 'cnote done' to stop editing.\n\r", ch); break;
          case NOTE_IDEA: send_to_char("Use 'idea done' to stop editing.\n\r", ch); break;
          case NOTE_PENALTY: send_to_char("Use 'penalty done' to stop editing.\n\r", ch); break;
          case NOTE_NEWS: send_to_char("Use 'news done' to stop editing.\n\r", ch); break;
          case NOTE_CHANGES: send_to_char("Use 'change done' to stop editing.\n\r", ch); break;
          case NOTE_OOC: send_to_char("Use 'onote done' to stop editing.\n\r", ch); break;
        	case NOTE_BUG: send_to_char("Use 'bug done' to stop editing.\n\r", ch); break;
        	case NOTE_QUEST: send_to_char("Use 'qnote done' to stop editing.\n\r", ch); break;
        }
        return;
      }
      if(!strcmp(argument, "done"))/* Done editing desc */
      {
        send_to_char("Done editing the note.\n\r", ch);
        end_long_edit(ch, &ch->pnote->text);
        return;
      }
      if(!strcmp(argument, "cancel"))/* Done editing desc - undo*/
      {
        send_to_char("Cancelled edits to the note.\n\r", ch);
        end_long_edit(ch, NULL);
        return;
      }
      if ( !str_cmp( argument, "post" ) || !str_cmp(argument, "send"))
      {
        send_to_char("Done editing the note.\n\r", ch);
        end_long_edit(ch, &ch->pnote->text);
      }
      ...
      {
        do_long_edit(ch, argument, LONG_EDIT_NOTE);
        return;
      }
    }
  ...
  return;
    ...
    if(!str_prefix(arg, "edit"))
    {
      ...
      if(!start_long_edit(ch, MAX_NOTE_DESC, LONG_EDIT_NOTE, ch->pnote->text))
        return;
      send_to_char("You are now in edit mode.\n\r", ch);
      return;
    }
...
void do_long_edit(CHAR_DATA *ch, char *arg, int type)
{
  int i;
  int last_word;
  char gap = ' ';
  ...
  {/* Line break */
    ...
    {/* Can't use if there is no text */
      ...
      send_to_char("You can't add line breaks without text above them.\n\r", ch);
  }
  else if(!strcmp(arg, "++"))
  {/* New paragraph */
    if(ch->pcdata->edit_len > 0)
    {/* Can't use if there is no text */
      ...
      ch->pcdata->edit_str[ch->pcdata->edit_len] = '\0';
      ch->pcdata->edit_count = 0;
      send_to_char("Paragraph break added.\n\r", ch);
    }
    else
      send_to_char("You can't add line breaks without text above them.\n\r", ch);
  }
  ...
  {/* Delete one line */
    if(ch->pcdata->edit_len <= 0)
      send_to_char("The text is empty.\n\r", ch);
    else
    {
      for(i = ch->pcdata->edit_len; i > 0; i--)
      {
  ...
  {/* Show the current description */
    if(ch->pcdata->edit_len <= 0)
      send_to_char("There is nothing in this text yet.\n\r", ch);
    else
    {
  ...
  {
    do_help(ch, "text edit");
  }
  else if(!strcmp(arg, "clear"))/* Erase */
  {
    ch->pcdata->edit_str[0] = '\0';
    ch->pcdata->edit_count = 0;
    ch->pcdata->edit_len = 0;
...
{// It is assumed buf is big enough to handle the set limit
  ...
  free_mem(ch->pcdata->edit_str, ch->pcdata->edit_limit + 2);

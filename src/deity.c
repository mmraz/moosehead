...
void do_pledge( CHAR_DATA *ch, char *argument )
{
  ...
  if(IS_NPC(ch))
  {
    send_to_char("NPCs don't need to change their deity.\n\r", ch);
    return;
  }
  if(IS_SET(ch->mhs, MHS_GLADIATOR))
  {
    send_to_char("Wait until you're done gladiating to change deities.\n\r", ch);
    return;
  }
  if(ch->pcdata->clan_info && ch->pcdata->clan_info->clan->type == CLAN_TYPE_FAITH)
  {
    send_to_char("Your deity is determined by your clan because it is a Clan of Faith.\n\r", ch);
    return;
  }
...
void do_pray( CHAR_DATA *ch, char *argument )
{
  ...
  if(IS_IMMORTAL(ch) && ch->level >= 55)
  {
    if(!str_cmp(arg, "rainbow"))
    {
      if(rainbow)
      {
        send_to_char("Only one rainbow at a time.\n\r", ch);
        return;
      }
      rainbow = 15;
      if(spawn_rainbow())
      {
        send_to_char("A rainbow appears in the world.\n\r", ch);
      }
      else
      {
        send_to_char("The requested rainbow failed to form.\n\r", ch);
      }
      return;
    }
  }
...
*/
  ...
  if(!str_prefix(arg, "intervene") || !str_prefix(arg, "intervention"))
  {
    ...
    if(ch->clan==nonclan_lookup("outcast") && ( ch->pcdata->outcT > 0))
    {
...
void give_gift(CHAR_DATA *ch,int gift)
{
  ...
  case 'k': /*knowledge*/
	  ...
	  if (d->character != NULL )
	    {
	    if (d->character->clan
		&& d->character->clan != nonclan_lookup("matook") 
		&& d->character->clan != nonclan_lookup("newbie") 
		&& !IS_IMMORTAL(d->character) )
            {
	      if (first)
	      {
...
void log_deity_favor(CHAR_DATA *ch, CHAR_DATA *alt, int type)
{
  char *strtime;
	...
	switch(type)
	{
		...
		case DEITY_TRIAL_FAIL_REQS: sprintf(buf, "%s %s Trial failed due to its reqs.", names, trial); break;
		default: sprintf(buf, "BUG with trial: %s %s %d.", names, trial, type); break;
	}

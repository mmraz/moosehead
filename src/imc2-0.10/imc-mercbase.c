...
static void send_rchannel(CHAR_DATA *ch, char *argument, int number)
{
  ...
  if (!str_cmp(arg, ",") || !str_cmp(arg, "emote"))
    imc_send_emote(getdata(ch), number, color_mtoi(arg2), imc_channels[chan].to);
  else
...
DEFINE_DO_FUN(do_rreply)
{
  ...
  imc_send_tell(getdata(ch), IMC_RREPLY(ch), color_mtoi(argument), 1);
...
DEFINE_DO_FUN(do_imclist)
{
  ...
  else if (!str_prefix(argument, "config"))
  {
    ...
    page_to_char("Unknown option.", ch);
...
#ifdef CIRCLE
  send_to_char((char *)imc_error(), ch);
#else
...
#ifdef CIRCLE
  send_to_char((char *)imc_ignore(argument), ch);
#else
...
#ifdef CIRCLE
  send_to_char((char *)imc_getstats(), ch);
#else
...
void imc_recv_whoisreply(const char *to, const char *text)
{
  ...
  for (d=descriptor_list; d; d=d->next)
  {
    ...
    {
#ifdef CIRCLE
      send_to_char((char *)color_itom(text), victim);
#else
